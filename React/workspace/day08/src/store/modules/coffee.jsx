...
const initialState = {
    items: [
        { id: 1, name: '아메리카노', price: 1500, qty: 0 },
        { id: 2, name: '라떼', price: 2300, qty: 0 },
        { id: 3, name: '카푸치노', price: 2000, qty: 0 },
        { id: 4, name: '모카', price: 2500, qty: 0 },
        { id: 5, name: '딸기라떼', price: 3000, qty: 0 },
        { id: 6, name: '초코프라페', price: 3500, qty: 0 }
    ],
    show: false
};
...
리덕스 리듀서 `coffee`: 
이 리듀서는 Redux 상태와 액션을 인자로 받아 상태를 업데이트하는 함수입니다. Redux 상태는 항상 불변성을 유지해야 하며, 이 리듀서도 이 규칙을 따릅니다. 액션 객체에는 액션 타입(`action.type`)과 필요한 추가 데이터(`id`, `qty`)가 포함되어 있으며, 이를 기반으로 상태를 변경합니다. 상태 변경은 각 액션 타입별로 다르게 수행됩니다.
...
1. **`UPDATE_QTY`**:
   ...
   - **불변성 유지**: 리덕스에서 상태 변경 시 불변성을 유지해야 하므로, `state.items` 배열을 직접 수정하지 않고 새로운 배열을 생성하여 반환합니다. 이 새로운 배열에는 `qty`만 업데이트된 상품이 포함됩니다.

2. **`SUBMIT`**:
   - 주문 완료 상태를 설정하는 액션입니다.
   - `show` 값을 `true`로 설정하여 주문이 완료되었음을 나타냅니다.
   - `state.items.filter(item => item.qty > 0)`를 사용하여 수량(`qty`)이 0보다 큰 항목만 남기고, 수량이 0인 항목은 `items` 배열에서 제거합니다. 이렇게 하면 주문하지 않은 상품들은 영수증에서 제외되게 됩니다.
...
### 상태 관리와 불변성:
각 액션 타입에 따라 상태가 다르게 업데이트되며, Redux의 원칙을 따라 상태 불변성을 유지하기 위해 `...state`, `...item` 같은 스프레드 연산자를 사용하여 기존 객체의 복사본을 반환하는 방식을 사용합니다. 이는 리덕스 애플리케이션에서 상태 변화가 추적 가능하고 디버깅이 용이하도록 하기 위함입니다.
