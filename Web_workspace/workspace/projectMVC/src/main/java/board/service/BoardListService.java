...
public class BoardListService implements CommandProcess {
	...
	public String requestPro(HttpServletRequest request, HttpServletResponse response) throws Throwable {
		...
		// MySQL
		// 한 페이지에 보여줄 게시글의 개수를 나타내는 변수입니다.
		// 예를 들어, 한 페이지에 5개의 게시글을 보여주고 싶다면 endNum은 5로 설정됩니다.
		// endNum의 값에 따라 한 번에 보여줄 데이터의 양이 결정됩니다.
		int endNum = 5; // 한 페이지에 표시할 데이터(게시글 등)의 개수
		// 현재 페이지 번호에 따라 데이터를 가져올 시작 위치를 계산하는 부분입니다.
		// 'pg'는 현재 페이지 번호를 나타내는 변수입니다.
		// 예를 들어, 1페이지는 시작 위치가 0, 2페이지는 5, 3페이지는 10이 됩니다.
		// 이 값은 데이터베이스에서 LIMIT 또는 OFFSET과 같은 SQL 쿼리에서 사용될 수 있습니다.
		int startNum = pg * endNum - endNum; 
		// 'startNum'은 현재 페이지에 해당하는 데이터의 시작 위치를 계산합니다.
		// 예를 들어, pg가 1일 경우 startNum은 0 (1페이지의 첫 번째 데이터).
		// pg가 2일 경우 startNum은 5 (2페이지의 첫 번째 데이터).
		// pg가 3일 경우 startNum은 10 (3페이지의 첫 번째 데이터)입니다.
		// 이를 통해 데이터베이스에서 페이지네이션(페이징) 처리를 할 때,
		// 데이터를 어디서부터 가져올지 결정할 수 있습니다.
